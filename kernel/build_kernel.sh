...
CFLAGS="-m64 -ffreestanding -mno-red-zone -mno-sse -mno-sse2 -mno-mmx \
        ...
        -I. -I./Subsystems -I./Drivers -I./vmm"
...
# Files in Root
C_SOURCES_ROOT="kernel.c"

# Files in Subsystems
C_SOURCES_SUBS="Subsystems/libx.c Subsystems/messages.c Subsystems/permissions.c Subsystems/shell.c Subsystems/scheduler.c Subsystems/syscall.c Subsystems/module.c Subsystems/sblock.c Subsystems/idt.c Subsystems/handlers.c Subsystems/acpi.c Subsystems/apic.c Subsystems/spinlock.c Subsystems/ioapic.c"

# Files in Drivers
C_SOURCES_DRIVERS="Drivers/vga.c Drivers/serial.c Drivers/keyboard.c Drivers/timer.c"

# Files in VMM
C_SOURCES_VMM="vmm/vmm.c vmm/page_tables.c vmm/buddy.c vmm/pmm.c"

# Files in SMP
C_SOURCES_SMP="smp/smp.c"

# Combined Source List
C_SOURCES="$C_SOURCES_ROOT $C_SOURCES_SUBS $C_SOURCES_DRIVERS $C_SOURCES_VMM $C_SOURCES_SMP"
ASM_SOURCES="kernel_entry.asm Subsystems/interrupts.asm"
...
rm -f *.o Subsystems/*.o Drivers/*.o vmm/*.o ${KERNEL_ELF} ${KERNEL_BIN} kernel.map
...
# 4b. Assemble SMP Trampoline (Binary Format) & Link
echo -e "${YELLOW}Building SMP Trampoline...${NC}"
$AS -f bin smp/trampoline.asm -o smp/trampoline.bin
# Convert binary to object file exposing symbols:
# _binary_smp_trampoline_bin_start, _end, _size
objcopy -I binary -O elf64-x86-64 -B i386 smp/trampoline.bin smp/trampoline.o --redefine-sym _binary_smp_trampoline_bin_start=_binary_kernel_smp_trampoline_bin_start --redefine-sym _binary_smp_trampoline_bin_end=_binary_kernel_smp_trampoline_bin_end --redefine-sym _binary_smp_trampoline_bin_size=_binary_kernel_smp_trampoline_bin_size
...
# Add trampoline object to link list
OBJ_FILES="kernel_entry.o $C_OBJS ${ASM_OBJS//kernel_entry.o/} smp/trampoline.o"
