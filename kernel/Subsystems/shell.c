...
static void cmd_test_ipc(void); // Added test command
...
static void cmd_test_ipc(void) {
    if (!current_task) {
        vga_puts("Error: No current task.\n");
        return;
    }

    uint32_t pid = current_task->pid;
    const char* payload = "Security Test Payload"; // Length: 21
    size_t payload_len = strlen(payload);

    vga_puts("IPC Test: Sending message to self...\n");
    if (msg_send(pid, pid, MSG_TYPE_DATA, payload, payload_len) != 0) {
        vga_puts("IPC Test: Send failed.\n");
        return;
    }

    // Test 1: Buffer too small
    vga_puts("IPC Test: Receiving with small buffer (Expect Fail)...\n");
    struct {
        struct message msg;
        uint8_t extra[4]; // Small buffer
    } small_buf;

    // msg_receive takes max_size.
    // If we pass sizeof(small_buf), it should reject it because message needs more.

    int ret = msg_receive(pid, (struct message*)&small_buf, sizeof(small_buf));
    if (ret == MSG_ERR_BUFFER_TOO_SMALL) {
         vga_puts("IPC Test: Correctly rejected small buffer.\n");
    } else {
         vga_puts("IPC Test: FAILED (Did not reject small buffer, ret=");
         vga_puti(ret);
         vga_puts(")\n");
         // Wait, if it succeeded, it dequeued it. We can't run second test.
         // And it overwrote stack.
         return;
    }

    // Test 2: Buffer sufficient
    // We didn't dequeue the message above because it failed (per my implementation).
    // So it should still be there.

    vga_puts("IPC Test: Receiving with large buffer (Expect Success)...\n");
    struct {
        struct message msg;
        uint8_t extra[128];
    } large_buf;

    ret = msg_receive(pid, (struct message*)&large_buf, sizeof(large_buf));
    if (ret == 0) {
        vga_puts("IPC Test: PASSED\n");
    } else {
        vga_puts("IPC Test: FAILED (ret=");
        vga_puti(ret);
        vga_puts(")\n");
    }
}
