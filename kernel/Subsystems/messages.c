...
int msg_receive(uint32_t receiver, struct message* out_msg, size_t max_size) {
    ...
    // CRITICAL: Must disable interrupts before checking to avoid lost wakeup (if we were using sleep)
    // But since we are spinning with hlt, we rely on interrupts to wake us from hlt.
    while (queue->count == 0) {
        asm volatile("hlt");
    }
    ...
    // Security Check: Ensure buffer is large enough
    size_t required_size = sizeof(struct message) + msg->size;
    if (required_size > max_size) {
        return MSG_ERR_BUFFER_TOO_SMALL; // Caller must retry with larger buffer
    }

    // Copy to user provided buffer envelope
    memcpy(out_msg, msg, required_size);
