...
int msg_receive(uint32_t receiver, struct message* out_msg, size_t max_size) {
    ...
    // Ensure the user buffer is large enough for header + payload
    size_t required_size = sizeof(struct message) + msg->size;

    if (required_size > max_size) {
        // Fail securely: Do not copy data if it won't fit
        // Note: We leave the message in the queue so it can be retrieved with a larger buffer
        // (or we could drop it, but retrying is usually preferred)
